...
def log_query(ip, container_name,container_id,db_connection, query,rows,db):
    utc_now = datetime.utcnow()
    germany_tz = pytz.timezone('Europe/Berlin')
    germany_time = pytz.utc.localize(utc_now).astimezone(germany_tz)
    db.insert_log(ip, container_name, container_id,db_connection, query, germany_time, rows)